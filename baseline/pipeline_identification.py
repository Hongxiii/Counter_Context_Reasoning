...
def llama(model_path, images, captions, explanations, setting, withCoT, ispre):
    ...
    if setting == 'z':
        if withCoT == 'n':  # 不使用思维链
            ...
            for i, c in zip(images, captions):
                background = i.split('/')[-2]
                prompt = f'''You will be presented with a question.You should just answer \"Yes\", \"No\" .\nUSER: {c} Does this description match the background knowledge of \"{background}\"? \nASSISTANT:'''
    ...
    # 少样本设定提示词（2个样本）
        ...
        if withCoT == 'n':  # 不使用思维链
            ...
            for i, c in zip(images, captions):
                ...
                prompt = f'''You will be presented with a question.You just should answer \"Yes\", \"No\" .\nUSER: Here are some examples:\nQ:{caption_1} Does this description match the background knowledge of \"{background_1}\"?\nA:{explanation_1}\nQ:{caption_2} Does this description match the background knowledge of \"{background_2}\"?\nA:{explanation_2}\nQ:{c} Does this description match the background knowledge of \"{background_0}\"?\nASSISTANT:'''
                prompts.append(prompt)
    # 模型推理
    result = {}
    for image, prompt in zip(images, prompts):
        generation_output = generator.text_completion(
            [prompt],
            max_gen_len=10,
            temperature=0.9,
            top_p=0.6,
        )
        output = generation_output[0]['generation']
    ...
    # 结果保存
    with open("../results/pipeline_identification/"+model_path.split('/')[-1]+'-'+setting+'-'+withCoT+'-'+ispre+'.json', 'w') as file:
    ...
    # 少样本设定提示词（2个样本）
        ...
        if withCoT == 'n':  # 不使用思维链
            ...
            for i, c in zip(images, captions):
                ...
                caption_1, caption_2 = captions[index_1], captions[index_2]  # 样本描述
                explanation_1, explanation_2 = explanations[index_1], explanations[index_2]  # 样本解释
                prompt = f'''You will be presented with a question.You should answer \"Yes\", \"No\" .\nUSER: Here are some examples:\nQ:{caption_1} Does this description match the background knowledge of \"{background_1}\"?\nA:{explanation_1}\nQ:{caption_2} Does this description match the background knowledge of \"{background_2}\"?\nA:{explanation_2}\nQ:{c} Does this description match the background knowledge of \"{background_0}\"?\nASSISTANT:'''
                prompts.append(prompt)
    # 模型推理
    result = {}
    for image, prompt in zip(images, prompts):
...
def gpt(model_path, images, captions, explanations, setting, withCoT, ispre):
    ...
    if setting == 'z':
        if withCoT == 'n':  # 不使用思维链
            ...
            for i, c in zip(images, captions):
                background = i.split('/')[-2]
                prompt = f'''You will be presented with a question.You should just answer \"Yes\" or \"No\". Do not ASK! \nUSER: {c} Does this description match the background knowledge of \"{background}\"?\nASSISTANT:'''
                prompts.append(prompt)
        else:  # 使用思维链
            prompts = []
            for i, c in zip(images, captions):
                background = i.split('/')[-2]
                prompt = f'''You will be presented with a question.You should just answer \"Yes\" or \"No\" .\nUSER: {c} Does this description match the background knowledge of \"{background}\"? Let's think step by step.\nASSISTANT:'''
                prompts.append(prompt)
    # 少样本设定提示词（2个样本）
    else:
        if withCoT == 'n':  # 不使用思维链
...
PATH = {
    ...
    'GPT-3.5': 'sk-XXXXXXXXXXXXXXXXXXXXXXXXXXXX'
}
